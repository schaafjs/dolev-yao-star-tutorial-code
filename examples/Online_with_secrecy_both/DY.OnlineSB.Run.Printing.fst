module DY.OnlineSB.Run.Printing
...
let state_to_string b =
  ...
  | SendingPing p -> Some (Printf.sprintf "SendingPing [prin = (%s), n_a = (%s)]" p.sp_bob (bytes_to_string p.sp_n_a))
  | SendingAck a -> Some (Printf.sprintf "SendingAck [prin = (%s), n_a = (%s)]" a.sa_alice (bytes_to_string a.sa_n_a))
